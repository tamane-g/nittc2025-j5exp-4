...
class CountSubjectSchoolClassController extends Controller // ★ クラス名を修正（RoomsControllerに合わせて、単数形CountSubjectSchoolClassControllerに）
{
     ...
     */
    public function store(Request $request): RedirectResponse // 戻り値の型ヒントを追加
    {
     ...
     * @return \Illuminate\Http\RedirectResponse
     */
    public function update(Request $request, int $id): RedirectResponse // 戻り値の型ヒントを追加
    {
     ...
     * @return \Illuminate\Http\RedirectResponse
     */
    public function destroy(int $id): RedirectResponse // 戻り値の型ヒントを追加
    {
        $record = CountSubjectSchoolClass::findOrFail($id);
        $record->delete();
